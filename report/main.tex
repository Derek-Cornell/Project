...
\textbf{What we'd do differently.} Run many more seeds. ILI training takes only minutes per run, so 10+ seeds per cell would have been easy and would have tightened our error bars considerably. For larger datasets like Electricity where each run takes hours, 3--5 seeds is a reasonable trade-off, but on ILI there was no excuse. We would also have run DLinear on the same seeds as PatchTST to control for noise on both sides of the table.
